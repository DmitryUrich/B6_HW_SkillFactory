...
class Album(Base):
    """
    Описывает структуру таблицы album
    для хранения записей музыкальной библиотеки
    """
    __tablename__ = "album"
...
def save(year, artist, genre, album):
    """
    Сохраняет в баззу данных
    (год, артиста, жанр, название альбома),
    если данные введены неправильно
    выводит сообщение о какой ошибке идет речь
    """
    ...
    if saved_album is not None:
        raise AlreadyExists(f"Альбом уже существует и имеет #{saved_album.id}")
